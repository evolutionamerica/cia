...
function elaborarMandado()
{
    ...
    if ($("#rgPessoa").val().length != 0 && $("#nomePessoa").val().length != 0 && $("#nOficio").val().length != 0 && $('#tipoMandado').val() != "" && $("#respPrisao").val() != "") 
    {
        if ($("#codigoPenal input:checkbox:checked").length != 0) {
            ...
            $("#oficioN").append(`Ofício Nº <b>${$("#nOficio").val().toUpperCase()}</b>/2021 - AGI/EUA.`)
            $('#nomeResp').append($('#respPrisao').val())
            if($('#tipoMandado').val() == 'Prisão'){
                $('#tipMandado').append('MANDADO DE PRISÃO')
                $('#ref').append(`Referência: <b>Pedido de Prisão Nº${$("#nOficio").val().toUpperCase()}/2021 - AGI/EUA.</b>`)
                ...
                $('#ref').append(`Referência: <b>Pedido de Pedido de Busca e Apreensão de bens como motos, carros e casas, Pedido de Prisão Nº${$("#nOficio").val().toUpperCase()}/2021 - AGI/EUA.</b>`)
                ...
                $('p4').append('<b>DETERMINO</b> que após a apreensão, o veículo seja levado para a delegacia local com um delegado do CIA, lavrado o termo de apreensão, que a requerente seja notificado e que o veículo seja entregue a pessoa de Nome Delegado qual é responsável público pelos fatos.')
            }
            html2canvas(document.getElementById("container"), {
                allowTaint: true,
                scrollY: -window.scrollY
    ...
    if ($("#rgPessoa").val().length != 0 && $("#nomePessoa").val().length != 0 && $("#nOficio").val().length != 0 && $('#tipoMandado').val() != "" && $("#respPrisao").val() != "") 
    {
        if ($("#codigoPenal input:checkbox:checked").length != 0) {
            ...
            if($('#tipoMandado').val() == 'Prisão'){
                ...
                $('#tipMandado').append('MANDADO DE BUSCA E APREENSÃO')
                $('#ref').append(`Referência: <b>Pedido de Busca e Apreensão de bens como motos, carros e casas, Pedido de Prisão Nº${$("#nOficio").val().toUpperCase()}/2021 - AGI/EUA.</b>`)
                $("#p1").append(`Cumprimentando-o, encaminho a Vossa Senhoria esse documento de busca e apreensão, em razão do cumprimento de mandado judicial de busca e apreensão e prisão de <b>${$('#nomePessoa').val().toUpperCase()}</b>, RG: <b>${$('#rgPessoa').val().toUpperCase()}</b> atuado no(s) Artigo(s): <b>${codigosP}</b>os demais artigos serão ajustados no que for pego nos bens do mesmo..`)
                $("#p2").append(`MANDA a autoridade Policial, a quem esteja com autoridade para portar esse documento, indo por ele devidamente assinado que em cumprimento, dirija-se a DP da cidade, e aí sendo, <b>REALIZE BUSCA E APREENSÃO</b> nos Veículos e Propriedades do mesmo.`)
                $('p4').append('<b>DETERMINO</b> que após a apreensão, o veículo seja levado para a delegacia local com um delegado do CIA, lavrado o termo de apreensão, que a requerente seja notificado e que o veículo seja entregue a pessoa de Nome Delegado qual é responsável público pelos fatos.')
            }
                ...
                $('#ref').append(`Referência: <b>Pedido de Revisão de bens como motos, carros e casas, Processo Nº${$("#nOficio").val().toUpperCase()}/2021 - AGI/EUA.</b>`)
                ...
                $('p4').append('<b>DETERMINO</b> que após a constatação de seus bens, o réu seja levado para a delegacia local com um delegado do CIA, lavrado o termo de revisão, que o(a) requerente seja notificado(a) e que o veículo seja entregue a pessoa que se encontra responsável por esse presente documento, o qual é responsável público pelos fatos.')
            }
            html2canvas(document.getElementById("container"), {
                allowTaint: true,
                scrollY: -window.scrollY
